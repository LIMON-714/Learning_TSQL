...
INSERT INTO Sales.Orders(orderid, custid, 
empid, orderdate, requireddate, shippeddate, shipperid, freight, shipname, shipaddress, shipcity, shipregion, shippostalcode, shipcountry)
...
-- start work practies here
select empid,firstname+' '+lastname as full_name from HR.Employees;

-- add alother table column
select hr.Employees.empid,firstname+' '+lastname as full_name, orderid,custid
from HR.Employees, Sales.Orders;

-- rename table 
select empid, custid from Sales.Orders as E;

----- from 
select 'hasan'as firstName , 'habib' as lastName;
select 'hasan'as firstName , 'habib' as lastName , 'hasan'+' '+'habib' as fullName;

-- number count 

select 10+20 as number;
select 10 as number1, 20 as number2, 10+20 as totalValue;

--<expression as alias> here
...
-- 123 or my country (space) or symble column name hola [my country] squre bra used.
...
--emp date 
select empid , birthdate from HR.Employees;


-- cast function using change data type 

select cast('01' as int);
select cast('10.4' as real);


use TSQL2012;
---------------NULL work
select* from hr.Employees
where birthdate <'19581209';
...
ADD full_name VARCHAR(100);  -- adjust the length as needed
...
select empid,city, ISnull (region,0) from hr.Employees;

select empid,city, ISnull (region,'No Region') as Region from hr.Employees;

select empid,city, ISnull (region,'No Region') as region
from hr.Employees
where ISnull (region,'')='';

---- now we can work on sorting & filtering
select empid,firstname,lastname,firstname+' '+lastname as FullName,country,region,city
from HR.Employees
where country= 'USA';
...
select empid,firstname,lastname,firstname+' '+lastname as FullName,country,region,city
from HR.Employees
where region='WA'and region is null;

select empid,firstname,lastname,firstname+' '+lastname as FullName,country,region,city
from HR.Employees
where region='WA'and region is not null;

select empid,firstname,lastname,firstname+' '+lastname as FullName,country,region,city
from HR.Employees
where region!='WA' and  region is not null;

--which row first word D 
select empid,firstname,lastname,firstname+' '+lastname as FullName,country,region,city
from HR.Employees
...
--again
select* from Sales.Orders
where orderDate = '07/04/2006';

-- month or day which is cach 8month 7 day this is standerd formet
select* from Sales.Orders
where orderDate = '20060807';

-- 2006 order situation only
select* from Sales.Orders
where year (orderDate) = 2006;

--see july monthly report 2006
select* from Sales.Orders
where year(orderDate) = 2006 and MONTH (orderdate)= 7;

--may i see data augst13 to 23 in 2006
select* from Sales.Orders
where orderdate between '20060813' and '20060823';
